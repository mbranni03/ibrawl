...
//   blast = [0 … 1, angle, colour = Claw'd's, spark = true] KO'd: Claw'd is gone, only a burst of rays shooting toward angle is left where it was
...
function drawBlast(x, y, t, ang, colour = CLAWD, spark = true) {
  ...
  for (let k = 0; k < 11; k++) {
    ...
    ctx.strokeStyle = k % 2 ? colour : INK; ctx.lineWidth = 2 + 8 * (1 - Math.abs(k - 5) / 6);
    line(x, y, x + Math.cos(a) * l, y + Math.sin(a) * l, 1.5, 1);
  }
  if (spark) drawSpark(x, y, SPARK_R * (1 + 2 * e), t * 3);
  ctx.restore();
}
